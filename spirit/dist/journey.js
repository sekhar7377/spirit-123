...
const chronological=nodes.slice().sort((a,b)=>a.birth-b.birth),visibleCount=nodes.filter(n=>n.size>0).length;let lastFilm=-1,visibleRank=0;
chronological.forEach((n,rank)=>{const position=Math.min(visibleRank/visibleCount*films.length,films.length-.001),film=Math.floor(position);if(n.size>0)visibleRank++;n.image=film;n.birth=film*.62+(position-film)*.43;if(n.size>0&&film!==lastFilm){n.size=Math.max(n.size,34);lastFilm=film}});
...
const w=entry.clientWidth,h=entry.clientHeight,dpr=Math.min(devicePixelRatio,2);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr)}ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h);const t=reduced?20:time,zoom=1.1-.1*ease(t/16),sizeScale=Math.max(.58,Math.min(w/1100,1.25));
...
const pulse=1;ctx.strokeStyle=n.depth===0?'rgba(141,98,68,.56)':`rgba(153,113,82,${(n.depth<3?.42:.3)*pulse})`;ctx.lineWidth=n.depth===0?1.3:n.depth<3?.7:.45;ctx.beginPath();ctx.moveTo(parent.x,parent.y);if(growth===1)ctx.quadraticCurveTo(cx,cy,p.x,p.y);else{const ex=curve(growth);ctx.quadraticCurveTo(parent.x+(cx-parent.x)*growth,parent.y+(cy-parent.y)*growth,ex.x,ex.y)}ctx.stroke();
const q=growth<1?growth:(t*.17-n.birth*.17+1e3)%1,v=curve(q);ctx.shadowColor='#d4a461';ctx.shadowBlur=0;ctx.fillStyle=growth<1?'rgba(186,126,60,.9)':'rgba(186,126,60,.48)';ctx.beginPath();ctx.arc(v.x,v.y,n.depth<2?(growth<1?2.1:1.3):.65,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0});
...
ctx.save();ctx.globalAlpha=p.alpha*(n.size>30?.98:.93);ctx.shadowColor='rgba(47,34,23,.27)';ctx.shadowBlur=n.depth<2?10*p.scale:2;ctx.shadowOffsetY=n.depth<2?4*p.scale:1;ctx.fillStyle='#29251f';ctx.beginPath();corners.forEach((v,k)=>k?ctx.lineTo(v.x,v.y):ctx.moveTo(v.x,v.y));ctx.closePath();ctx.fill();ctx.shadowBlur=0;ctx.shadowOffsetY=0;
